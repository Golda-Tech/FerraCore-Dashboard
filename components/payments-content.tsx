...
export function PaymentsContent() {
  ...
  const fetchTodaySummary = useCallback(async (email: string) => {
    ...
    try {
      ...
      console.error("Failed to fetch today payments summary:", err);
    }
  ...
  }, [searchTerm, activeStatuses, appliedStartDate, appliedEndDate, currentPage, pageSize, fetchTodaySummary]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, appliedStartDate, appliedEndDate, pageSize, user?.email]);
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Based on {todayTotalCount} transaction{todayTotalCount !== 1 ? "s" : ""} today
            </p>
        ...
        <CardContent>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4">
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              <span>
                Showing {rangeStart}–{rangeEnd} of {totalRequests} payments
              </span>
