...
const authV2Endpoints: EndpointDef[] = [
  {
    ...
    auth: "basic",
    headers: [
      { key: "Authorization", value: "Basic <Base64(subscriptionKey:subscriptionSecret)>", description: "Base64-encoded subscription_key:subscription_secret. Pre-request script builds this automatically." },
    ],
    responseExample: {
      status: 200,
      body: JSON.stringify(
...
const paymentV2Endpoints: EndpointDef[] = [
  {
    ...
    description:
      "Phase 1 — Pre-processing. Enriches the payment request with account name (via name enquiry) and a 5-minute expiry window. Stores the pre-payment record in both the database and Redis. Caller identity (partner email and organisation) is resolved automatically from the JWT — do NOT send initiatedBy or initiationPartnerId. If auto_execute is true, the payment is submitted immediately and returns a PAYMENT_SUBMITTED response.",
    auth: "bearer",
    headers: [
      { key: "Authorization", value: "Bearer YOUR_ACCESS_TOKEN" },
      { key: "Content-Type", value: "application/json" },
      { key: "X-Reference-Id", value: "{{x_reference_id}}", description: "Optional — auto-generated by the pre-request script if omitted. Used to track the pre-payment through execute/cancel." },
      { key: "X-Callback-Url", value: "https://your-callback-url.com/webhook", description: "Optional — your webhook URL for payment status notifications." },
    ],
    requestBody: {
      raw: JSON.stringify(
        {
      ...
      description:
        "Caller identity is resolved from the JWT — do not include initiatedBy or initiationPartnerId. Set auto_execute to true to submit the payment immediately (skips the Execute step).",
    },
    queryParams: [
      { key: "merchant_reference", type: "string", required: true, description: "Your unique merchant reference. 1–50 chars, letters/digits/hyphen/underscore." },
      { key: "msisdn", type: "string", required: true, description: "Customer phone number — exactly 10 digits, e.g. 0241234567." },
      { key: "network", type: "string", required: true, description: "Payment network — one of: MTN, VOD, AIR, GMO." },
      { key: "amount", type: "number", required: true, description: "Amount in GHS — positive, max 2 decimal places." },
      { key: "description", type: "string", required: false, description: "Optional payment description — max 100 chars." },
      { key: "auto_execute", type: "boolean", required: false, description: "If true, bypasses the 2-phase flow and submits the payment immediately. Default: false." },
    ],
    responseExample: {
      status: 200,
      body: JSON.stringify(
      ...
      description:
        "The reference_id in the response body is the UUID to use in Execute/Cancel. Response headers also include X-Reference-Id (same UUID), X-Ref-Status: VALID, and X-Ref-Expires (ISO-8601 expiry timestamp). When auto_execute is true, the response is { merchant_reference, code: \"PAYMENT_SUBMITTED\", message }.",
    },
    notes:
      "The pre-payment expires after 5 minutes. You must call Execute Payment within this window using the reference_id returned in this response. Response headers: X-Reference-Id, X-Ref-Status (VALID), X-Ref-Expires.",
  },
  {
    ...
    headers: [
      { key: "Authorization", value: "Bearer YOUR_ACCESS_TOKEN" },
      { key: "X-Reference-Id", value: "{{x_reference_id}}", description: "Required — use the reference_id from the Initiate Payment response body." },
    ],
    responseExample: {
      status: 200,
      body: JSON.stringify(
    ...
    notes:
      "The pre-payment must be in CREATED status and within the 5-minute expiry window. Error codes: PAYMENT_ALREADY_CANCELLED (409), PAYMENT_ALREADY_EXECUTED (409), PAYMENT_WINDOW_EXPIRED (410 Gone), NOT_FOUND (404), EXECUTION_FAILED (500).",
  },
  {
    ...
    notes:
      "Error codes: PAYMENT_ALREADY_EXECUTED (409), PAYMENT_ALREADY_CANCELLED (409), NOT_FOUND (404). Only pre-payments in CREATED status can be cancelled.",
  },
  {
    ...
    description:
      "Queries the payment provider for the final outcome of a submitted payment. All provider outcomes (successful, pending, failed) return HTTP 200 with a machine-readable code. Only request-level errors (bad params, not found) return 4xx.",
    auth: "bearer",
    headers: [{ key: "Authorization", value: "Bearer YOUR_ACCESS_TOKEN" }],
    queryParams: [
      { key: "network", type: "string", required: true, description: "Payment network — one of: MTN, VOD, AIR, GMO." },
      { key: "transactionRef", type: "string", required: true, description: "Transaction reference — 1–50 chars, letters/digits/hyphens/underscores." },
    ],
    responseExample: {
      status: 200,
      body: JSON.stringify(
        {
          merchant_reference: "MERCH-001",
          transactionRef: "TXN-20250101-000001",
          network: "MTN",
      ...
      description:
        "code is one of: PAYMENT_SUCCESSFUL, PAYMENT_PENDING, PAYMENT_FAILED — all return HTTP 200. Error codes: TRANSACTION_NOT_FOUND (404), INVALID_INPUT (400).",
    },
    notes:
